...
public class RoundedManhattanConnectionRouter extends AbstractRouter {
	...
	public void route(Connection conn) {
		...
		if (start.y >= end.y)
			if (start.x <= end.x)
				offset = 0; //target oben rechts von
						 // source
			else
				offset = 1; //target oben links von source
		else if (start.x <= end.x)
			offset = 2; //target unten rechts von source
		else
			offset = 3; //target unten links von source
		if (rs.x + rs.width / 2 <= start.x) //out rechts
			if (rt.x + rt.width / 2 >= end.x)
				offset += 0; //in links
			else
				offset += 4; //in rechts
		else //out links
		...
		if (Math.abs(start.x - end.x) >= 20 && Math.abs(start.y - end.y) >= 20)
			...
			r1 = Math.min(Math.abs(start.x - end.x) / 2, Math.abs(start.y - end.y) / 2);
		if (Math.abs(start.y - end.y) >= 20)
			r2 = 10;
		else
			r2 = Math.abs((Math.abs(start.y - end.y)) / 2);
		...
		case 1: {
			if (source.equals(target)) {
				...
				if (Math.abs(start.y - end.y) >= 40) {
					...
					PointList p2 = leftCorner(new Point(start.x + 20, end.y - (end.y - start.y) / 2),
							SWT.BOTTOM, 10);
					PointList p3 = rightCorner(new Point(end.x - 20, end.y - (end.y - start.y) / 2),
							SWT.RIGHT, 10);
					PointList p4 = rightCorner(new Point(end.x - 20, end.y), SWT.BOTTOM, 10);
					for (i = 0; i < p1.size(); i++)
						l.addPoint(p1.getPoint(i));
					...
					PointList p3 = halfleftCircle(new Point(start.x + 20, start.y), SWT.LEFT, Math
							.abs(start.y - end.y) / 4);
					PointList p4 = halfrightCircle(new Point(end.x - 20, end.y - (end.y - start.y) / 2),
							SWT.RIGHT, Math.abs(start.y - end.y) / 4);
					for (i = 0; i < p3.size(); i++)
						l.addPoint(p3.getPoint(i));
					for (i = 0; i < p4.size(); i++)
						l.addPoint(p4.getPoint(i));
		...
		case 3: {
			if (Math.abs(start.y - end.y) >= 40) {
				...
				PointList p3 = halfrightCircle(new Point(start.x + 20, start.y), SWT.LEFT, Math.abs(start.y
						- end.y) / 4);
				PointList p4 = halfleftCircle(new Point(end.x - 20, end.y - (end.y - start.y) / 2), SWT.RIGHT,
		...
		case 12: {
			if (source.equals(target)) {
				...
				if (Math.abs(start.y - end.y) >= 40) {
					...
					PointList p2 = rightCorner(new Point(start.x - 20, end.y - (end.y - start.y) / 2),
							SWT.BOTTOM, r2);
					PointList p3 = leftCorner(new Point(end.x + 20, end.y - (end.y - start.y) / 2),
							SWT.LEFT, r2);
					...
					PointList p3 = halfrightCircle(new Point(start.x - 20, start.y), SWT.RIGHT, Math
							.abs(start.y - end.y) / 4);
					PointList p4 = halfleftCircle(new Point(end.x + 20, end.y - (end.y - start.y) / 2),
							SWT.LEFT, Math.abs(start.y - end.y) / 4);
					for (i = 0; i < p3.size(); i++)
						l.addPoint(p3.getPoint(i));
					for (i = 0; i < p4.size(); i++)
						l.addPoint(p4.getPoint(i));
		...
		case 14: {
			if (Math.abs(start.y - end.y) >= 40) {
				...
				PointList p3 = halfleftCircle(new Point(start.x - 20, start.y), SWT.RIGHT, Math.abs(start.y
						- end.y) / 4);
				PointList p4 = halfrightCircle(new Point(end.x + 20, end.y - (end.y - start.y) / 2), SWT.LEFT,
						Math.abs(start.y - end.y) / 4);
				for (i = 0; i < p3.size(); i++)
					l.addPoint(p3.getPoint(i));
				for (i = 0; i < p4.size(); i++)
