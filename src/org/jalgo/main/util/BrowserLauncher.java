// $Id: BrowserLauncher.java,v 1.1 2005/06/23 10:09:52 jalgosequoia Exp $
...
public class BrowserLauncher {
	...
	private static final String NETSCAPE_OPEN_PARAMETER_END = ",new-tab)'";
	...
	private static Object locateBrowser() {
		...
		switch (jvm) {
			...
			case OTHER:
			default:
				String[] possibleMozillaNames = { "firefox", "mozilla" };
			
				for (int i = 0; i < possibleMozillaNames.length; i++) {
					try {
						Process mozilla = Runtime.getRuntime().exec(possibleMozillaNames[i] + " -v");
						if (mozilla.waitFor() == 0) {
		...
		switch (jvm) {
		    ...
		    case WINDOWS_9x:
				...
				*/
				break;
			case OTHER:
				// Assume that we're on Unix and that Mozilla is installed
				
				// First, attempt to open the URL in a currently running session of Mozilla
