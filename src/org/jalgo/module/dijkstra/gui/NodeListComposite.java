 ...
 * 
 * The NodeListComposite provides a textfield an a button to edit the nodelist
 * for the graph
 */
...
extends ControllerComposite {
	...
	protected class ApplyNodeListSelectionAdapter
	...
	protected class NodeListObserver
	implements Observer {
	...
	public NodeListComposite(Controller ctrl, Composite arg0, int arg1) {
		...
		t.addMouseTrackListener(new StatusbarTextMouseTrackAdapter(
			getController(), Messages.getString(
				"dijkstra", "NodeListComposite.Tooltip"))); //$NON-NLS-1$ //$NON-NLS-2$

		Button b = new Button(this, SWT.CENTER);
		b.setText(Messages.getString("dijkstra", "NodeListComposite.Apply")); //$NON-NLS-1$ //$NON-NLS-2$
		b.addSelectionListener(new ApplyNodeListSelectionAdapter(
			this.getController(), t));
		b.addMouseTrackListener(new StatusbarTextMouseTrackAdapter(
			getController(), Messages.getString(
				"dijkstra", "NodeListComposite.Apply_description"))); //$NON-NLS-1$ //$NON-NLS-2$
