 ...
 * $Id: DijkstraAlgorithm.java,v 1.4 2005/10/03 10:34:12 stephancr Exp $
 */
...
public class DijkstraAlgorithm {
	...
	private State createState(ArrayList border, String descr,
			boolean macro, Node activeNode) {
		return createState(border, descr, macro, activeNode, null);
	}

	private State createState(ArrayList border, String descr,
			boolean macro, Node activeNode, Edge skipEdge) {
		...
		Edge nskipEdge = (skipEdge != null) ? ngraph.findEdge(ngraph
				.findNode(skipEdge.getStartNodeIndex()), ngraph
				.findNode(skipEdge.getEndNodeIndex())) : null;
		...
		for (Edge edge : graph.getEdgeList()) {
			...
			if (other.getChosen() == false) {
				if (other.getDistance() == INFINITY) {
					...
					edge.setActive(true);
					edge.setBorder(true);
					edge.setReversed(other == edge
							.getStartNode());
					State c1 = createState(border, "",
							false, other, edge);
					addNodeReference(c1, other);
					c1.addDescription(" ist über ");
					addNodeReference(c1, node);
					c1
							.addDescription(" auf einem kürzeren Wege erreichbar.");
					...
					addNodeReference(c1, other);
					c1
							.addDescription(" ist \u00FCber den bestehenden Weg k\u00FCrzer zu erreichen.");
				...
				other.setDistance(node.getDistance()
						+ edge.getWeight());
				if (!border.contains(other))
					border.add(other);

				// Create state
				State st1 = createState(border,
						"Neuer Knoten im Rand: ",
						false, other);
		...
		for (Node bnode : border) {
			...
			if (pred == null) {
				throw new RuntimeException(
						"Predecessor of border node is null");
			}
			if (bnode == pred) {
				throw new RuntimeException(
						"Node equals predecessor");
			}
	...
	public void generateStates(Node start) {
		...
		createState(
				border,
				"Randknotenmenge f\u00FCr Startknoten komplett.",
				true);
		...
		while (!border.isEmpty()) {
			...
			State state1 = createState(border,
					"Neuer gew\u00E4hlter ", true, v);
	...
	private void dumpResult(Node start) {
		System.out.println("Dumping results for Node "
				+ start.getIndex());

		for (Node node : graph.getNodeList()) {
			System.out.println("Node " + node.getIndex()
					+ " distance = " + node.getDistance());
			for (Node pred = node; pred != null; pred = pred
					.getPredecessor())
