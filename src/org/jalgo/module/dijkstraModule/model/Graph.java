...
public class Graph implements Serializable, Cloneable
{
	 ...
	 */
	public Graph(ArrayList nodes, ArrayList edges)
	{
		nodeList = nodes;
		edgeList = edges;
	}
	...
	public Object clone() {
		...
		for (int i = 0; i<nodeList.size(); i++)
			newGraph.addNode((Node)((Node)nodeList.get(i)).clone());
		
		for (int i = 0; i<nodeList.size(); i++) {
		    Node n = (Node)newGraph.nodeList.get(i);
		...
		for (int i = 0; i<edgeList.size(); i++)
			newGraph.addEdge((Edge)((Edge)edgeList.get(i)).clone());
		for (int i = 0; i<edgeList.size(); i++) {
		    Edge e = (Edge)newGraph.edgeList.get(i);
	...
	public boolean addNode(Node node)
	{
		for(int i=0; i < nodeList.size(); i++)
		{
			if (((Node) nodeList.get(i)).getIndex() == node.getIndex())
			{
				return false;
			}
		}
	...
	public boolean addEdge(Edge edge)
	{
		...
		for(int i=0; i < edgeList.size(); i++) // check if edge already exists
		{
			...
			if(currentEdge.equals(edge))
			{
				...
				if (newWeight == currentEdge.getWeight()) return false;
		...
		{
			if (((Node) edge.getStartNode()).getIndex() == ((Node) nodeList.get(i)).getIndex())
			{
				edge.setStartNode((Node) nodeList.get(i));		
			}
			if (((Node) edge.getEndNode()).getIndex() == ((Node) nodeList.get(i)).getIndex())
			{
				edge.setEndNode((Node) nodeList.get(i));		
			}
		}
	...
	public String getEdgeListText()
	{
		...
		if (edgeList.size() > 0)
		{
			returnValue = returnValue + ((Edge) edgeList.get(0)).getText();
		}
		for(int i=1; i < edgeList.size(); i++)
		{
			returnValue = returnValue + ", " + ((Edge) edgeList.get(i)).getText();
		}
	...
	public String getNodeListText()
	{
		...
		if (nodeList.size() > 0)
		{
			returnValue = returnValue + ((Node) nodeList.get(0)).getIndex();
		}
		for(int i=1; i < nodeList.size(); i++)
		{
			returnValue = returnValue + ", " + ((Node) nodeList.get(i)).getIndex();
		}
	...
	public Graph rescale()
	{
		...
		if (numberOfNodes == 1) {
			...
			((Node) nodeList.get(0)).setPosition(new Position(0.0, 0.0));
		}
		else {
			for (int i = 0; i < numberOfNodes; i++)
			{
				...
				((Node) nodeList.get(i)).setPosition(newPosition);
			}
		}
	...
	{
		int nodeToChange = 0, startNodeIndex, endNodeIndex;
		Node currentNode;
		Edge currentEdge;
		ArrayList newNodes = new ArrayList(10);
		ArrayList badNodes = new ArrayList(10);
		...
		for (int i=0; i < edgeList.size(); i++)
		{
			...
			if (startNodeIndex != -1)
			{
				currentEdge.setStartNode((Node) newNodes.get(startNodeIndex));
			}
			
			endNodeIndex = badNodes.indexOf(currentEdge.getEndNode());
			if (endNodeIndex != -1)
			{
				currentEdge.setEndNode((Node) newNodes.get(endNodeIndex));
			}
		}
	}
	...
	public boolean deleteNode(Node nodeToDelete)
	{
		for (int i=0; i <= nodeList.size(); i++)
		{
			if(((Node) nodeList.get(i)).getIndex() == nodeToDelete.getIndex())
			{
				/*
				 * FSt
			     ...
			     */
			    nodeList.remove((Node)nodeList.get(i));
				//~FSt
				...
				while (stillHasEdges)
				{
					...
					{
						if ((nodeToDelete.getIndex() == ((Edge) edgeList.get(j)).getStartNode().getIndex()) || (nodeToDelete.getIndex() == ((Edge) edgeList.get(j)).getEndNode().getIndex()))
						{
							currentEdge = (Edge) edgeList.get(j);
							stillHasEdges = true;
						}
					}
					if (stillHasEdges) this.deleteEdge(currentEdge);
				}
				return true;
			}
		}
	...
	public void setAllChangedFlagsFalse()
	{
		for (int i=0; i < nodeList.size(); i++)
		{
			((Node) nodeList.get(i)).setChanged(false);
		}
		
		for (int i=0; i < edgeList.size(); i++)
		{
			((Edge) edgeList.get(i)).setChanged(false);
		}
	}
	...
	public void synchronizeWith(Graph anotherGraph, boolean deleteNodes)
	{
		...
		for (int i=0; i < nodeList.size(); i++)
		{
			currentNode = (Node) nodeList.get(i); // get an element from this Graph
			
			for (int j=0; j < anotherNodeList.size(); j++)
			{
				if (currentNode.getIndex() == ((Node) anotherNodeList.get(j)).getIndex()) deleteIt = false;
			}
			if (deleteIt) nodesToDelete.add(currentNode);
		...
		for (int i=0; i < edgeList.size(); i++)
		{
			currentEdge = (Edge) edgeList.get(i); // get an element from this Graph
			
			for (int j=0; j < anotherEdgeList.size(); j++)
			{
				if (currentEdge.equals((Edge) anotherEdgeList.get(j))) deleteIt = false;
			}
			if (deleteIt) edgesToDelete.add(currentEdge);
		...
		{
			this.deleteEdge((Edge) edgesToDelete.get(i));
		}
		...
		if (deleteNodes)
		{
			...
			{
				this.deleteNode((Node) nodesToDelete.get(i));
			}
		}
