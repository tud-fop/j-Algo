...
public class MatrixComposite extends ControllerComposite {
		...
		public void focusLost(FocusEvent e) {
			...
			if (m_arTextfields[r][s].getText().length() == 2) {
				m_arTextfields[s][r].setText(m_arTextfields[r][s]
															   .getText().replace('-', ' '));
				m_arTextfields[r][s].setText(m_arTextfields[r][s]
															   .getText().replace('-', ' '));
	...
	public MatrixComposite(Controller ctrl, Composite arg0, int arg1) {
		...
		compo.setLayout(l);
		compo.addMouseTrackListener(new StatusbarTextMouseTrackAdapter(getController(),"Eingabe von Kantengewichten zwischen je zwei Knoten. Gewichte werden automatisch zur Hauptdiagonale symmetrisch erg\u00E4nzt, da Graph ungerichtet ist. Kante l\u00F6schen durch Eingabe von Minus (-)."));
		...
		button.addSelectionListener(new ApplyMatrixButtonSelectionAdapter(this,
				this.getController()));
		button.addMouseTrackListener(new StatusbarTextMouseTrackAdapter(getController(),"\u00C4nderungen an der Matrix \u00FCbernehmen und Graph neu darstellen."));
		...
		for (int k = 0; k < 9; k++) {
			for (int i = 0; i < 9; i++) {
				...
				m_arTextfields[r][s].addFocusListener(new TextFocusAdapter(
						m_arTextfields, r, s));
				m_arTextfields[r][s].addMouseTrackListener(new StatusbarTextMouseTrackAdapter(getController(),"Eingabe von Kantengewichten zwischen je zwei Knoten. Gewichte werden automatisch zur Hauptdiagonale symmetrisch erg\u00E4nzt, da Graph ungerichtet ist. Kante l\u00F6schen durch Eingabe von Minus (-)."));
				...
				m_arTextfields[r][s].addKeyListener(new KeyAdapter() {
					...
					public void keyReleased(KeyEvent e) {
						...
						boolean notnull = (!(m_arTextfields[s][r].getText()
								.equals("")));
						notbackspace=true;
						if (nottab && notnull && notcursorleft && notcursorup && notcursordown && notcursorright && notbackspace) {
							w = s;
							...
							if (nottab && notcursorleft && notcursorup && notcursordown && notcursorright && notbackspace) {
								m_arTextfields[s][r].setText("-");
								m_arTextfields[r][s].setText("-");
							}
						}
	...
	public String getEdgeList() {
		...
		for (int m = 0; m < 9; m++) {
			if (nodeExist(m + 1)) {
				...
				for (int n = (0); n < 9; n++) {
					boolean notminus = (!(m_arTextfields[n][m].getText()
							.equals("-")));
					boolean notnull = (!(m_arTextfields[n][m].getText()
							.equals("")));
					if (notminus && (!(n == m)) && notnull) {
						edgeList = edgeList + "(" + (m + 1) + ","
						+ m_arTextfields[n][m].getText() + ","
						+ (n + 1) + "),";
						
					}
					if (!notminus) {
						
						edgeList = edgeList + "(" + (m + 1) + "," + "1,"
						+ (m + 1) + "),";
