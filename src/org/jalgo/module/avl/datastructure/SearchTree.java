...
/**
 * The class <code>SearchTree</code> is the main data structure of the AVL module.
 * It holds a reference to the root node and provides several methods for
 * working with the tree. Some methods provide information about the tree, such as
 * the weight, the height, the level of a specified node and so on. Other methods
 * provide traversion of the tree in different orders.
 *  
...
public class SearchTree {
	...
	/**
	 * Constructs a new <code>SearchTree</code> object with the given root node.
	 * This root node can represent a complete partial tree, but attention:
	 * ensure to calculate the height of the tree!
	 * 
	...
	/**
	 * Prints this <code>SearchTree</code> in inorder to the standard output stream.
	...
	/**
	 * Returns the height of this <code>SearchTree</code>. This method does not
	 * calculate the height. It only returns a precalculated value.
	 * So all algorithms, which can change the structure of the tree, have to
	 * calculate the height newly.<br>
	 ...
	 * interesting for displaying the tree and for calculating the average height
	 * of this <code>SearchTree</code>.
	 * 
	 ...
	 *
	 * @return the <code>Node</code>s of this <code>SearchTree</code> in inorder
	 */
	...
	/**
	 * A helper method to get an inorder list of the <code>Node</code>s of this
	 ...
	 * <code>Node</code>s in a list.
	 *
	 * @return the <code>Node</code>s of this <code>SearchTree</code> in postorder
	 */
	...
	/**
	 * A helper method to get an postorder list of the <code>Node</code>s of this
	 * <code>SearchTree</code>.
	 * 
	 * @param node the currently watched node in traversion
	 * @param list an empty <code>Node</code> list to be filled
	 ...
	 * <code>Node</code>s in a list.
	 * This method is currently interesting for serializing the
	 * <code>SearchTree</code>.
	 *
	 * @return the <code>Node</code>s of this <code>SearchTree</code> in levelorder
	 */
	 ...
	 * Traverses this <code>SearchTree</code> in levelorder and puts the
	 * <code>Node</code>s in a list.
	 * This list has 2^(getHeight()-1)-1 entries. So it can contain many
	 * <code>null</code> values, but such an uncompressed list is interesting
	 * e.g. for displaying the tree.
	 *
	...
	/**
	 * A helper method to get a levelordered list of the <code>Node</code>s of this
	 * <code>SearchTree</code>. This list is riddled because of the possible
	 * unbalanced structure of the tree.
	 * The list to be filled has to be of the following type:
	 * <code>Node[(1&lt;&lt;getHeight())-1]</code>
	 * 
	 * @param node the currently watched node in traversion
	 * @param list the <code>Node</code> array to be filled
	 * @param index the index of the currently watched node,
	 * 				important for level ordering
	 */
	...
	/**
	 * Fills this <code>SearchTree</code> with the given keys, so that the structure
	 * of the tree is supported.
	 * Attention: the <code>SearchTree</code> is not cleared before inserting the
	 * values. If there are entries in the tree, they are keeped. If You want to
	 * use this operation to create a new tree with the given values, e.g. during
	...
	public void importLevelOrder(List<Integer> levelOrder) {
		...
		for (int key : levelOrder) {
			...
			while (insert.hasNext()) insert.perform();
		}
...
//		calcAllBalances.perform();
	...
	//TODO: raus mit dieser methode! dafür haben wir ein command.
	//avltest, left- und rightrotation können das command benutzen.
	...
	/**
	 * Retrieves the node with the minimal key in the partial tree with the given
	 * root node.
	 * 
