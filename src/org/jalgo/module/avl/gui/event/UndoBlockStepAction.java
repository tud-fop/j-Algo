...
/**
 * The class <code>UndoBlockStepAction</code> defines an <code>Action</code>
 * object, which can be added to toolbars or menus. Performing this action
 * causes the currently running algorithm to undo a block step.
 * 
...
extends SwingSWTAction {
	...
	public UndoBlockStepAction(GUIController gui, Controller controller) {
		...
		setText(Messages.getString("avl", "Undo_blockstep")); //$NON-NLS-1$ //$NON-NLS-2$
		setToolTipText(Messages.getString("avl", "Undo_blockstep_tooltip")); //$NON-NLS-1$ //$NON-NLS-2$
		setIconImage(Messages.getResourceURL("avl", "Undo_blockstep")); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	public void run() {
		...
		if (!controller.algorithmHasNextStep())
			gui.algorithmRestartedFromUndo();

		try {
			controller.undoBlockStep();
		}
