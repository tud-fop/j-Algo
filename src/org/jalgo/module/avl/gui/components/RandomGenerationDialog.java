...
/**
 * The class <code>RandomGenerationDialog</code> represents a dialog for creating
 * a random tree. It provides the setting of the number of nodes, the AVL property
 * and the type of visualization. The three available visualization types are
 * defined as constants in <code>GUIConstants</code>.<br>
 * The input of the number of nodes is validated on the fly and appropriate messages
 * are displayed to the user, if the input is not valid.<br>
 * To show the dialog, call <code>open()</code>, all settings are reset before. 
 * 
...
implements GUIConstants {
	...
	public RandomGenerationDialog(GUIController gui, Frame parent,
		...
		super(parent, "Zufallsbaum erstellen", true);
...
//			getClass().getResource("/pix/avl/msg_error.gif")));
		...
		JLabel countLabel = new JLabel("Anzahl der Knoten:      ");
		...
		add(textField);
		JLabel avlLabel = new JLabel("AVL - Eigenschaft:");
		...
		GUIController.setGBC(ok, gbl, 0, 6, 2, 1,
	 ...
	 * 
	 * @return <code>true</code>, if AVL mode is selected,
	...
	public int validateInput() {
		...
		try {
			...
			if (currentInput > MAX_KEY ||
				currentInput < MIN_KEY) returnCode = NOT_IN_RANGE;
		...
		case VALID_INPUT:
			...
			break;
		case NO_INTEGER:
			setErrorMessage("Bitte nur ganze Zahlen eingeben!");
			setOKEnabled(false);
			break;
		case NOT_IN_RANGE:
			setErrorMessage("Nur Werte von "+MIN_KEY+" bis "+MAX_KEY+" eingeben!");
			setOKEnabled(false);
			break;
		case INPUT_EMPTY:
			setErrorMessage(null);
	 ...
	 * 
	 * @return <code>NO_VISUALIZATION</code> - if the tree should be generated
	 * 											in background
	 *         <code>STEPWISE</code> - if the generation should be confirmed
	 *         									stepwise by the user
	 *         <code>AUTOMATICAL</code> - if the generation of the tree should
	 *         									run as an animation
	 */
