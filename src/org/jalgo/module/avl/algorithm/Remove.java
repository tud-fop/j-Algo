	...
	public void perform() {
		...
		if (c instanceof SearchAlg) {
			...
			case FOUND:
				...
				nodeToDelete.setVisualizationStatus(Visualizable.FOCUSED | Visualizable.LINE_NORMAL);
				if (nodeToDelete.getRightChild()==null)
					commands.add(CommandFactory.createRemoveNode(wn, tree, nodeToDelete));
				else
					commands.add(CommandFactory.createFindSuccessor(wn));
				break;
			case WORKING:
		...
		if (c instanceof RemoveNode) {
			...
			results.set(1, "remove4");
			results.set(2, DONE);
			//ich hab folgende zeile hier eingefügt, weil ich sonst exceptions
...
//			results.add(3, wn.getNextToMe());
	...
	public void undo() {
		...
		else if ((c instanceof FindSuccessor) && !((FindSuccessor)c).hasPrevious()) {
			currentPosition--;
			c = commands.get(currentPosition);
		}
	...
	public void undoBlockStep() {
		...
		if (c instanceof SearchAlg) 
			while (((SearchAlg)c).hasPrevious())
				undo();
		else if (c instanceof FindSuccessor)
			while (((FindSuccessor)c).hasPrevious())
				undo();
		else
			c.undo();
