 ...
 * 
 * The class <code>InsertAVL</code> realizes the insert of a node in an AVL tree.
 * It uses the classes <code>Insert</code> to find the position and insert the key.
 * It then uses the classes <code>UpdateBalance</code>, <code>RotateLeft</code> or <code>
 * RotateRight</code> to restore the AVL characteristic.
 */
...
public class InsertAVL extends MacroCommand implements Constants{
	...
	public void undo() {
		...
		if (c instanceof UpdateBalance) {
			...
			// necessary for DoubleRotation, sets old RotateVisualisationStatus back to Normal
