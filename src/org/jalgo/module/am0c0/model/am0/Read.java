...
public class Read extends IOStatement {
	 ...
	 *             if target < 0
	 */
	public Read(LineAddress address, int target) {
		super(address);
	...
	public MachineConfiguration apply(MachineConfiguration configuration)
			throws IllegalArgumentException {
		if (!configuration.getInputStream().stream.isEmpty()) {
			configuration.getProgramCounter().inc();
			configuration.getRam().store(target,
					configuration.getInputStream().read());
			return configuration;
		} else
			throw new IllegalArgumentException(Messages.getString(
					"am0c0", "Read.1")); //$NON-NLS-1$
	}

	@Override
	public String getDescription() {
		return Messages.getString("am0c0", "Read.2"); //$NON-NLS-1$
	}

	@Override
	public String getCodeText() {
		return "READ " + target + ";"; //$NON-NLS-1$ //$NON-NLS-2$
	}
}
