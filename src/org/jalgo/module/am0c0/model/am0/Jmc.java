 ...
 * @author David Voigt
 */
public class Jmc extends JumpStatement {
	private int target;
	...
	public MachineConfiguration apply(MachineConfiguration configuration)
		...
		if (configuration.getStack().getStackAsList().size() < 1) {
			throw new IllegalArgumentException(
					Messages.getString("am0c0", "Jmc.0")); //$NON-NLS-1$
		}
	...
	public String getDescription() {
		return Messages.getString("am0c0", "Jmc.1"); //$NON-NLS-1$
	}

	@Override
	public String getCodeText() {
		return "JMC " + target + ";"; //$NON-NLS-1$ //$NON-NLS-2$
	}
