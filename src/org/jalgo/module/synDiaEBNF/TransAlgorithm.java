...
/**
 * Represents the algorithm for transforming from an EBNF term to a syntactical diagram.
 * The algorithm is described in the A&D script of Prof. Vogler from the 
 * 7th Oct. 2002 at page 141.
 * 
...
public class TransAlgorithm implements IAlgorithm, Serializable {
	...
	private final int ALGO_POS_REPETITION = 4;			// position in txtCanvas for EbnfRepetition
	private final int ALGO_POS_OPTION = 5;					// position in txtCanvas for EbnfOption
	private final int ALGO_POS_PRECEDENCE = 6;			// position in txtCanvas for EbnfPrecedence
	private final int ALGO_POS_TWO_ALPHAS = 7;			// position in txtCanvas for the case of two alphas
	private final int ALGO_POS_CONCATENATION = 8;	// position in txtCanvas for EbnfConcatenation
	private final int ALGO_POS_ALTERNATIVE = 9;		// position in txtCanvas for EbnfAlternative

	private ModuleController moduleController;						// instance of current module controller
	private TextCanvas txtCanvas;										// canvas to display the algorithm as described in the lecture 
	private Figure gfxCanvas;													// canvas to display the grafical figures 
	private StackCanvas toTransCanvas;								// canvas to display which syntactical variables still have to be transformed

	private SynDiaSystem synDiaSystem;							// abstract representation of the generated synDiaSystem which also holds
	...
	private boolean auto; 														// true if the algorithm should perform automatically, otherwise false
	 ...
	 * 
	 * @param mainWin the main Window, where the Algorithm can put its own Windows
	 ...
	 */
	public TransAlgorithm(	ModuleController moduleController,
											Figure gfxCanvas,
											TextCanvas txtCanvas,
											StackCanvas toTransCanvas,
											EbnfDefinition ebnfDef,
		...
		txtCanvas.setMarkStyle(new MarkStyle(SynDiaColors.textHighlight, SynDiaColors.diagramNormal, 3));
		...
		if (tVarList.getFirst() == ebnfDef.getStartVariable())						// S already on top?
			...
			int i = 1;
			do {
				if (tVarList.get(i) == ebnfDef.getStartVariable()) {
					tVarList.set(i, tVarList.getFirst());
					tVarList.set(0, ebnfDef.getStartVariable());
					finished = true;
				}
				++i;
			} while (!finished);
		...
		// create new InitialFigure labeled with the name of the new syntactical diagram
		InitialFigure newInitialFigure = new InitialFigure(newEbnfSynDia.getLabel());
		newInitialFigure.replace(newTransFigure, 0);
		// create new abstract initial element by passing its figure and its inner element
	...
	/**
	* @return true  if you can perform the next step in the algorithm, 
	*               so you can call performNextStep()
	*         false if not
	*/
	 ...
	 * It restores the next saved step of the visualisation.
	 * 
	 * @exception IndexOutOfBound if there is no previous step to go
	 */
	 ...
	 * It restores the last saved step of the visualisation.
	 * 
	 * @exception IndexOutOfBound if there is no previous step to go
	 */
	...
	/**
	 * Perfoms the whole Algorithm automatically. 
	 * Nondeterministic decissions are made randomly.
	 */
	...
	private void showDialogFinished() {
		MessageDialog.openInformation( toTransCanvas.getShell(), Messages.getString("TransAlgorithm.Algorithm_completed_19"), Messages.getString("TransAlgorithm.The_algorithm_has_been_completed_!_20")); //$NON-NLS-1$ //$NON-NLS-2$
	}

	private void highlightTxtAndElem() {
		txtCanvas.demarkAll();
				
		SynDiaToTrans trans = (SynDiaToTrans)stillToTrans.peak();
	 ...
	 * It realises the transAlgorithm. <p>
	...
	public void performNextStep() throws IndexOutOfBoundsException, SynDiaException {
		...
		// *** EbnfSynVariable ***
		if (trans.getEbnf() instanceof EbnfSynVariable) {
			// create new abstract variable and its corresponding grafical object
			SynDiaVariable newVariable = new SynDiaVariable(((EbnfSynVariable)trans.getEbnf()).getLabel(), gfxCanvas.getFont());
			newVariable.setGfx(new VariableFigure(((EbnfSynVariable)trans.getEbnf()).getLabel(), gfxCanvas.getFont()));
			// store this variable to set its startElem befor finishing the algorithm
			alreadyCreatedVariables.add(newVariable);
			
			if (parentToTrans.peak() instanceof SynDiaInitial) {							// parent is a SynDiaInitial?
			...
			else if (parentToTrans.peak() instanceof SynDiaAlternative) {			// parent is a SynDiaAlternative?
				SynDiaAlternative topElem = (SynDiaAlternative)parentToTrans.peak();
				int index = 0;
				if (topElem.getOption(0) != trans)
					index = 1;
				if (index == 1 || (topElem.getOption(1) instanceof SynDiaEpsilon))
					topElem = (SynDiaAlternative)parentToTrans.pop();
				topElem.setOption(index, newVariable);
				topElem.getGfx().replace(newVariable.getGfx(), index);
			}
			else if (parentToTrans.peak() instanceof SynDiaConcatenation) {	// parent is a SynDiaConcatenation?
				SynDiaConcatenation topElem = (SynDiaConcatenation)parentToTrans.peak();
				int index = 0;
				while (topElem.getContent(index) != trans)
					++index;
				if (index == topElem.getNumOfElements() - 1)
					topElem = (SynDiaConcatenation)parentToTrans.pop();
				topElem.setContent(index, newVariable);
				topElem.getGfx().replace(newVariable.getGfx(), index);
			}
			else if (parentToTrans.peak() instanceof SynDiaRepetition) {			// parent is a SynDiaRepetition?
				SynDiaRepetition topElem = (SynDiaRepetition)parentToTrans.pop();
		...
		//	*** EbnfTerminal ***
			...
			// create a new abstract terminal and its corresponding grafical object
			SynDiaTerminal newTerminal = new SynDiaTerminal(((EbnfTerminal)trans.getEbnf()).getLabel(), gfxCanvas.getFont());
			newTerminal.setGfx(new TerminalFigure(newTerminal.getLabel(), gfxCanvas.getFont()));
		...
		// *** EbnfRepetition ***
			...
			// create new SynDiaToTrans element as the repeated element and set its grafical representation (ToTransFigure)
			SynDiaToTrans newTrans = new SynDiaToTrans(((EbnfRepetition)trans.getEbnf()).getContent());
			ToTransFigure newTransFigure = new ToTransFigure(newTrans.getLabel(), gfxCanvas.getFont());
			newTrans.setGfx(newTransFigure);
			// create the corresponding grafical representation of the new SynDiaToTrans, straightAheadElem is an EmptyFigure
			...
			SynDiaRepetition newRepetition = new SynDiaRepetition(newRepetitionFigure, new SynDiaEpsilon(), newTrans);
		...
		// *** EbnfOption ***
			...
			// create new SynDiaToTrans element
			SynDiaToTrans newTrans = new SynDiaToTrans(((EbnfOption)trans.getEbnf()).getContent());
			ToTransFigure newTransFigure = new ToTransFigure(newTrans.getLabel(), gfxCanvas.getFont()); 
			newTrans.setGfx(newTransFigure);
			// create the corresponding grafical representation of the new SynDiaToTrans, repeated element is an EmptyFigure
		...
		// *** EbnfPrecedence ***
			...
			// create new abstract SynDiaToTrans
			SynDiaToTrans newTrans = new SynDiaToTrans(((EbnfPrecedence)trans.getEbnf()).getContent());
			newTrans.setGfx(new ToTransFigure(newTrans.getEbnf().toString(), gfxCanvas.getFont()));
		...
		// *** EBNFConcatenation *** 
			...
			// create and set the corresponding ToTransFigure with its label representing the EBNF-term to trans
			// also replace the corresponding CloudFigure by a ToTransFigure
			// add the new SynDiaToTrans to the list
			for (int i = 0; i < ((EbnfConcatenation)trans.getEbnf()).getNumOfElements(); i++) {
				SynDiaToTrans newTrans = new SynDiaToTrans(((EbnfConcatenation)trans.getEbnf()).getContent(i));
				ToTransFigure newTransFigure = new ToTransFigure(newTrans.getLabel(), gfxCanvas.getFont()); 
				newTrans.setGfx(newTransFigure);
				newConcatenationFigure.replace(newTransFigure, i);	
				elements.add(newTrans);
			}
			// create a new abstract concatenation (SynDiaConcatenation) by passing its figure and its elements
			SynDiaConcatenation newConcatenation = new SynDiaConcatenation(newConcatenationFigure, elements);
			// push all new SynDiaToTrans elements of the new abstract concatenation onto the stack, the first will be ontop afterwards
		...
		// *** EbnfAlternative ***
			...
			// create a new abstract alternative with two new SynDiaToTrans elements together with its grafical objects
			SynDiaAlternative newAlternative = new SynDiaAlternative();
			// abstract left with grafical object
			SynDiaToTrans newTransLeft = new SynDiaToTrans(((EbnfAlternative)trans.getEbnf()).getLeft());
			ToTransFigure newTransFigureLeft = new ToTransFigure(newTransLeft.getLabel(), gfxCanvas.getFont());
			newTransLeft.setGfx(newTransFigureLeft);
			newAlternative.addOption(newTransLeft);
			// abstract right with grafical object
			SynDiaToTrans newTransRight = new SynDiaToTrans(((EbnfAlternative)trans.getEbnf()).getRight());
			ToTransFigure newTransFigureRight = new ToTransFigure(newTransRight.getLabel(), gfxCanvas.getFont());
	...
	/**
	 * Writes all necessary objects to a stream for serialization.
	 * When chanching here also change readObject() !
	 */
	...
	/**
	 * Reads ealier serialized objects from stream and restores class.
	 * When chanching here also change writeObject() !
	 */
