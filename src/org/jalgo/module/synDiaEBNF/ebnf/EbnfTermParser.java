 ...
 */
public class EbnfTermParser
extends Parser
implements IEbnfTokenConstants {

	private Stack<StackElement> parseStack;
	private EbnfDefinition definition;
	private String variable;
	private Set<String> terminals, variables;
	 ...
	 */
	public EbnfTermParser(String term, EbnfDefinition definition,
		String variable, Set<String> variables, Set<String> terminals) {
		Set<String> tokens = new HashSet<String>(variables);
		tokens.addAll(terminals);
		tokenizer = new EbnfTokenizer(term, tokens);
	...
	throws EbnfParseException {
		...
		while (tokenizer.hasNextToken()) {
			...
			if (tName.equals(SYMBOL)) {
				tokenizer.consume();
				parseStack.push(new StackElement(SYMBOL, token.getTokenValue(),
					level));
				concatenation(level);
				return;
			}
				...
				if (tName.equals(RIGHT_CURLY_BRACKET)) {
					tokenizer.consume();
					parseStack.push(new StackElement(REPETITION, token
					.getTokenValue(), level));
					concatenation(level);
					return;
				}
				...
				if (tName.equals(RIGHT_SQUARED_BRACKET)) {
					tokenizer.consume();
					parseStack.push(new StackElement(OPTION, token
					.getTokenValue(), level));
					concatenation(level);
					return;
				}
				...
				if (tName.equals(PIPE)) {
					tokenizer.consume();
					parseStack.push(new StackElement(PIPE, token
					.getTokenValue(), level));
					begin(level + 1);
					concatenation(level + 1);
					if (!tokenizer.hasNextToken()) { throw new EbnfParseException(
					...
					if (tName.equals(RIGHT_BRACKET)) {
						tokenizer.consume();
						parseStack.push(new StackElement(ALTERNATIVE, token
						.getTokenValue(), level));
						concatenation(level);
						return;
					}
					...
					tokenizer.consume();
					parseStack.push(new StackElement(PRECEDENCE, token
					.getTokenValue(), level));
					concatenation(level);
					return;
				}
	...
	private void concatenation(int level)
	throws EbnfParseException {
		Token token = null;
		Integer tName = new Integer(0);
		if (tokenizer.hasNextToken()) {
			...
			if (tName.equals(SYMBOL)) {
				...
				if (tName.equals(RIGHT_CURLY_BRACKET)) {
					...
					.getTokenValue(), level));
					parseStack
					.push(new StackElement(CONCATENATION, null, level));
					concatenation(level);
					return;
				}
				throw new EbnfParseException(errorMsg(Messages
				...
				if (tName.equals(PIPE)) {
					...
					if (tName.equals(RIGHT_BRACKET)) {
						...
						parseStack.push(new StackElement(ALTERNATIVE, token
						.getTokenValue(), level));
						parseStack.push(new StackElement(CONCATENATION, null,
							level));
						concatenation(level);
						return;
					}
			...
			else if (tName.equals(RIGHT_CURLY_BRACKET)
				|| tName.equals(RIGHT_SQUARED_BRACKET)
				|| tName.equals(RIGHT_BRACKET) || tName.equals(PIPE)) { return; }
		}
	}

	private EbnfElement buildEbnfTree(int level)
	throws EbnfParseException {
		while (!parseStack.isEmpty()) {
			StackElement elem = parseStack.peek();
			if (elem.getType().equals(CONCATENATION)
				&& elem.getLevel() == level) {
				parseStack.pop();
				EbnfConcatenation c = new EbnfConcatenation();
				while (!parseStack.isEmpty()
					&& parseStack.peek().getLevel() == level) {
					if (parseStack.peek().getType().equals(CONCATENATION)) {
						parseStack.pop();
						continue;
					}
			...
			else if (elem.getType().equals(ALTERNATIVE)
				&& elem.getLevel() == level) {
				parseStack.pop();
			...
			else if (elem.getType().equals(PRECEDENCE)
				&& elem.getLevel() == level) {
				parseStack.pop();
				return new EbnfPrecedence(buildEbnfTree(level + 1));
			}
			else if (elem.getType().equals(REPETITION)
				&& elem.getLevel() == level) {
				parseStack.pop();
