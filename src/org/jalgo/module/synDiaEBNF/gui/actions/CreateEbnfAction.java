 ...
 */
public class CreateEbnfAction extends Action
		implements
			IModeConstants,
			Serializable {
		...
		if (mc.getMode() != NORMAL_VIEW_EMPTY) {
			if (!(MessageDialog
					.openQuestion(
							parent.getShell(),
							Messages
									.getString("CreateEbnfAction.Really_create_new_EBNF__4"), //$NON-NLS-1$
							Messages
									.getString("CreateEbnfAction.Creating_a_new_EBNF_will_destroy_all_you_have_done_with_this_module_up_to_now_!_5")))) { //$NON-NLS-1$
				return;
			}
		}
