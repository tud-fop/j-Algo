 ...
 *
 */
public class AddConcatenationAction extends Action implements
		IClickAction<Figure> {
	...
	public AddConcatenationAction(IFigure figure) {
		...
		setImageDescriptor(ImageDescriptor.createFromURL(getClass()
				.getResource("/ebnf_pix/concatenation.gif")));
	}
	...
	public void performAction(List<Figure> items) {
		...
		if (inDialog.open() != Window.CANCEL) {
			try {
				result = (Integer.valueOf(inDialog.getValue()))
						.intValue();
			...
			try {
				((CompositeSynDiaFigure) help.getParent())
						.replace(
								(SynDiaFigure) help,
