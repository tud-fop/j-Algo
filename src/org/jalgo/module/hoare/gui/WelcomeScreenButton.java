...
/**
 * The class <code>WelcomeButton</code> represents a rollover-enabled
 * graphical button with a description, which can be displayed on mouse
 * hovering.
 * 
 * @author Markus
 */
public class WelcomeScreenButton extends JToggleButton {
	...
	/**
	 * Constructs a <code>WelcomeButton</code> object with the given
	 * parameters. The description string is represented as a bitmap image,
	 * because of better layout on different platforms and the missing
	 * antialiasing of text on Swing components.
	 * 
	 * @param defaultIcon
	 *            the icon, which is displayed, when button is not selected
	 * @param rolloverIcon
	 *            the icon, which is displayed on mouse hovering
	 * @param description
	 *            the description string as image
	 * @param actionCommand
	 *            the action command
	 * @param action
	 *            an event handler implementing <code>ActionListener</code>
	...
	public WelcomeScreenButton(ImageIcon defaultIcon, ImageIcon rolloverIcon,
		...
		// of welcome screen still highlights the button, even if mouse is not
		// over
