...
public class Write extends IOStatement {
	 ...
	 *             if target < 0
	 */
	public Write(LineAddress address, int target) {
		super(address);
	...
	public MachineConfiguration apply(MachineConfiguration configuration)
		...
		if (configuration.getRam().cellIsDefined(target)) {
			...
			configuration.getOutputStream().write(
					configuration.getRam().load(target));
			return configuration;
		} else
			throw new IllegalArgumentException(Messages.getString(
					"am0c0", "Write.0")); //$NON-NLS-1$
	}

	@Override
	public String getDescription() {
		return Messages.getString("am0c0", "Write.1"); //$NON-NLS-1$
	}

	@Override
	public String getCodeText() {
		return "WRITE " + target + ";"; //$NON-NLS-1$ //$NON-NLS-2$
	}
