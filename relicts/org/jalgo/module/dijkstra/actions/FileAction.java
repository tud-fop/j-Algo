...
extends GraphAction {
	...
	throws ActionException {
		...
		else if (open()) getController().setGraph(m_newGraph);
		return true;
	}

	public boolean undoAction()
	throws ActionException {
		if (m_bSave == false) getController().setGraph(m_oldGraph);
	 ...
	 * 
	 */
	protected boolean save(Graph g)
	throws ActionException {
		...
		final String[] FILTER = {
			Messages.getString("dijkstra", "FileAction.Graph_object"), Messages.getString("dijkstra", "FileAction.All_files")}; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$

		dialog.setFilterNames(FILTER);
		dialog.setFileName("*.gra"); //$NON-NLS-1$
	...
	throws ActionException {
		...
		final String[] Ext = {"*.gra"}; //$NON-NLS-1$
