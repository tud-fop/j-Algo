...
extends ControllerComposite {
	...
	protected class GraphCompositeObserver
	implements Observer {
	...
	protected class RescaleGraphButtonAdapter
	...
	public GraphComposite(Controller controller, Composite composite,
		int style, Device device, boolean bEditMode) {
		...
		rescaleGraphButton.addSelectionListener(new RescaleGraphButtonAdapter(
			controller));
		rescaleGraphButton.setText(Messages.getString(
			"dijkstra", "GraphComposite.Arrange_nodes_automatically")); //$NON-NLS-1$ //$NON-NLS-2$
		rescaleGraphButton.setLayoutData(getButtonGridData());
		rescaleGraphButton.addMouseTrackListener(
			new StatusbarTextMouseTrackAdapter(getController(), Messages.getString(
				"dijkstra", "GraphComposite.Arrange_tooltip"))); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private GridData getCanvasGridData() {
		return new GridData(GridData.FILL_BOTH | GridData.GRAB_HORIZONTAL
